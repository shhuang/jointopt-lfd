        ...
        for i_step in range(args.num_steps):
            ...
            for _ in range(args.lookahead_depth):
                ...
                for (q, a, tf, r_a) in agenda:
                    ...
                    success, bodypart2trajs = simulate_demo_fn(cur_xyz, actionfile[a], animate=False)
                    if args.animation:
                        Globals.viewer.Step()
            ...
            success, trajs = simulate_demo_fn(new_xyz, actionfile[best_root_action], animate=args.animation)
